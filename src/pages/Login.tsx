...
const CircuitDecorations = () => (
  <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
    ...
    <svg className="absolute top-4 left-4 w-32 h-32 opacity-10" viewBox="0 0 128 128" fill="none">
      <motion.path d="M0 64 H32 L40 56 H72 L80 64 H112" stroke="hsl(var(--primary))" strokeWidth="0.8"
        animate={{ pathLength: [0, 1, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "linear" }} />
      <motion.path d="M32 0 V24 L40 32 V56" stroke="hsl(var(--primary))" strokeWidth="0.8"
        animate={{ pathLength: [0, 1, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "linear", delay: 1 }} />
      <motion.path d="M80 128 V100 L72 92 V64" stroke="hsl(var(--accent))" strokeWidth="0.5"
        animate={{ pathLength: [0, 1, 0] }} transition={{ duration: 6, repeat: Infinity, ease: "linear", delay: 2 }} />
    ...
    <svg className="absolute bottom-4 right-4 w-32 h-32 opacity-10 rotate-180" viewBox="0 0 128 128" fill="none">
      <motion.path d="M0 64 H32 L40 56 H72 L80 64 H112" stroke="hsl(var(--accent))" strokeWidth="0.8"
        animate={{ pathLength: [0, 1, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "linear", delay: 0.5 }} />
    ...
    <motion.svg className="absolute top-8 right-8 w-20 h-20 opacity-[0.07]" viewBox="0 0 80 80" fill="none"
    ...
    <motion.svg className="absolute bottom-8 left-8 w-16 h-16 opacity-[0.06]" viewBox="0 0 64 64" fill="none"
    ...
    <svg className="absolute left-0 top-1/2 -translate-y-1/2 w-4 h-48 opacity-15" viewBox="0 0 16 192" fill="none">
      {[0, 1, 2, 3, 4, 5].map(i => (
        <motion.line key={i} x1="8" y1={i * 32 + 8} x2="8" y2={i * 32 + 24}
    ...
    <motion.div className="absolute top-1/3 left-0 w-full h-px"
...
const GlitchLogo = () => {
  ...
  return (
    <div className="relative w-36 h-36 mx-auto mb-4 overflow-hidden">
      ...
      <motion.img src={logoImage} alt="" className="absolute inset-0 w-full h-full object-contain pointer-events-none"
        ...
        animate={{
          x: glitch ? intensity * 2.5 : 0, opacity: glitch ? 0.5 : 0,
      ...
      <motion.img src={logoImage} alt="" className="absolute inset-0 w-full h-full object-contain pointer-events-none"
        style={{ mixBlendMode: "screen" }}
        animate={{
          x: glitch ? -intensity * 3 : 0, opacity: glitch ? 0.4 : 0,
...
/* ── Mini Radio Player ── */
const MiniRadioPlayer = ({ onAnalyserReady }: { onAnalyserReady: (a: AnalyserNode) => void }) => {
    ...
    <div className="w-full mt-6 p-3 rounded-md border border-border/50 relative overflow-hidden"
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[11px] font-display tracking-wider text-foreground truncate">{track.title}</p>
          {track.artist && <p className="text-[9px] tracking-widest text-muted-foreground truncate mt-0.5">{track.artist}</p>}
          <p className="text-[8px] tracking-[0.3em] text-primary/50 mt-1 font-display">{playing ? "▶ ON AIR" : "■ OFFLINE"}</p>
        </div>
...
const Login = ({ onLogin }: { onLogin: () => void }) => {
  ...
  const handleAnalyser = useCallback((a: AnalyserNode) => {
    analyserNodeRef.current = a;
  }, []);
  ...
  return (
    <div className="fixed inset-0 bg-background flex flex-col items-center justify-center overflow-hidden z-50">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.5 }} className="relative z-10 w-full max-w-sm mx-4">
        <div className="p-6 rounded-md border border-border/50 relative overflow-hidden"
          ...
          <form onSubmit={handleSubmit}>
            <motion.div animate={shaking ? { x: [-8, 8, -6, 6, -3, 3, 0] } : {}} transition={{ duration: 0.4 }}>
              <div className="relative mb-4">
                ...
                <input type="password" value={password}
                  ...
                  style={{ borderColor: error ? "hsl(var(--destructive))" : "hsl(var(--border))", color: "hsl(var(--foreground))" }}
            ...
            <button type="submit"
              ...
              style={{ background: "hsl(var(--primary) / 0.1)", borderColor: "hsl(var(--primary) / 0.4)", color: "hsl(var(--primary))" }}
              onMouseEnter={e => { e.currentTarget.style.background = "hsl(var(--primary) / 0.2)"; e.currentTarget.style.boxShadow = "0 0 16px hsl(var(--primary) / 0.3)"; }}
              onMouseLeave={e => { e.currentTarget.style.background = "hsl(var(--primary) / 0.1)"; e.currentTarget.style.boxShadow = "none"; }}>
              <LogIn className="w-3.5 h-3.5" />
              ENTRAR
            </button>
          </form>

          <MiniRadioPlayer onAnalyserReady={handleAnalyser} />
